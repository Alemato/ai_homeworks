...
class MinMaxAlphaBetaPruningHlCut:
    ...
    def __init__(self, game, heuristic, h0_cut, k=5, l=3, max_depth=1):
        ...
        self.eval_hl_cut_count = 0
        self.prune_hl_cut_count = 0
        self.memoization = {}
    ...
    def __hl_cut(self, states, turn):
        initial_count = len(states)  # Conta il numero iniziale di neighbors
        for state in states:
            state.hl = self.__minmax_alpha_beta_hl(state, self.l - 1, float("-inf"), float("inf"),
                                                   not turn)
        sorted_states = sorted(states, key=lambda state: state.hl, reverse=turn)[:self.k]
        self.prune_hl_cut_count += initial_count - len(sorted_states)  # Aggiorna prune_hl_cut_count
        return sorted_states

    def __minmax_alpha_beta_hl(self, state, depth, alpha, beta, turn):
        self.eval_hl_cut_count += 1  # Count the number of state evaluations.
        ...
        if turn:  # Maximizing player
            ...
            for neighbor in neighbors:
                ...
                alpha = max(alpha, value)  # Update alpha with the maximum value found so far.
                if alpha >= beta:  # Alpha-Beta pruning: Stop evaluating if alpha is greater than or equal to beta.
                    self.prune_hl_cut_count += 1  # Count pruned branches.
                    break
            return value
        else:  # Minimizing player
            value = float("inf")
    ...
    def search(self, state: StateChessGame):
        neighbors = self.game.neighbors(state)  # Generate possible successor states.
        top_neighbors = self.__hl_cut(neighbors, state.game_board.turn)  # Valuta, riordina e taglia i neighbors
